require 'helper'
require 'openstack_compute'

class TestOpenStackCompute < Minitest::Test
  ...
  def setup
    ...
    @mock_conn.expect :openstack_compute_client, @mock_client
  end

  def test_that_id_returns_id_directly_when_constructed_with_an_id
    assert_equal Id, OpenStackCompute.new(Id, @mock_conn).id
  end
  ...
  def test_that_id_returns_fetched_id_when_constructed_with_a_name
    ...
    assert_equal Id, OpenStackCompute.new({ name: 'cut' }, @mock_conn).id
  end
  ...
  def test_that_instance_returns_instance_when_instance_exists
    ...
    assert_same mock_instance, OpenStackCompute.new(Id, @mock_conn).send(:instance)
  end

  def test_that_instance_returns_nil_when_instance_does_not_exist
    @mock_resource.expect :instance, nil, [Id]
    assert OpenStackCompute.new(Id, @mock_conn).send(:instance).nil?
  end
  ...
  def test_that_exists_returns_true_when_instance_exists
    ...
    @mock_resource.expect :instance, mock_instance, [Id]
    assert OpenStackCompute.new(Id, @mock_conn).exists?
  end
